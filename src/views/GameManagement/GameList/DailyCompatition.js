...
const DailyCompetition = ({ playerData }) => {
  const [FileUpload, setFileUpload] = useState(null);
  const Grade = [1, 2, 3, 4, 5, 6, "Others"];
  const [gradeValue, setGradeValue] = useState("");
  const [gameCategory, setGameCategory] = useState("");

  const handleDailyCompetition = (event) => {
    ...
    fetch(`${config.apiServer}/dailycomp/register`, {
      method: "POST",
      body: formData,
    })
      .then((res) => res.json())
  ...
  return (
    <div>
      <MainCard title="Daily Competition(Add Question)">
        <form onSubmit={handleDailyCompetition}>
          <Box>
...
export default DailyCompetition;
